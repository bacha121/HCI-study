...
function Radar({ u, dims, size = 220 }) {
  ...
  return (
    <svg width="100%" height="100%" viewBox="0 0 220 220" preserveAspectRatio="xMidYMid meet" style={{ maxWidth: size, display:"block", margin:"0 auto", overflow: "visible" }}>
      ...
      {dims.map((d, i) => {
        const p = pt(i, 1.34);
        const ly = p[1] + (Math.abs(p[1]-cy) < 2 ? 4 : p[1] > cy ? 10 : 0);
        return <text key={i} x={p[0]} y={ly} textAnchor={labelAnchor(p[0])} fontSize={11} fontWeight={600} fill={u.text2} fontFamily={L.font}>{d.l}</text>;
      })}
    </svg>
  );
}
